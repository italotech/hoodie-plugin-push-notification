  ...
  MessagesAPI.send = function (notificationType, from, to, callback) {
    ...
    if (templateMessages[notificationType]) {
      _getProfile([from, to], function (err, profiles) {
        async.parallel([
          async.apply(_sendMessage, templateMessages[notificationType].to, from, profiles),
          async.apply(_sendMessage, templateMessages[notificationType].from, to, profiles)
        ], function (err) {
          console.log('done');
        });
      });
